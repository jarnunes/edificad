...
create table configuracao (token_expires_at integer, create_date timestamp(6), id bigint not null, last_modified_date timestamp(6), create_user varchar(255), last_modified_user varchar(255), token_secret_key varchar(255), primary key (id));
...
create table eds_role_user (create_date timestamp(6), id bigint not null, last_modified_date timestamp(6), user_id bigint, create_user varchar(255), last_modified_user varchar(255), role varchar(255) not null check (role in ('ADMIN','OPERATOR','WEBSERVICES')), primary key (id));
create table eds_user (enabled boolean not null, locked boolean not null, create_date timestamp(6), id bigint not null, last_modified_date timestamp(6), create_user varchar(255), email varchar(255) not null unique, full_name varchar(255) not null, last_modified_user varchar(255), password varchar(255) not null, username varchar(255) not null unique, primary key (id));
create table eds_password_reset_token (create_date timestamp(6), expiry_date timestamp(6) not null, id bigint not null, last_modified_date timestamp(6), user_id bigint not null unique, create_user varchar(255), last_modified_user varchar(255), token varchar(255) not null unique, primary key (id));
create table endereco (estado varchar(2) not null check (estado in ('MG','SP','RJ')), create_date timestamp(6), id bigint not null, last_modified_date timestamp(6), cep varchar(14), cidade varchar(50), bairro varchar(100), numero varchar(100) not null, create_user varchar(255), last_modified_user varchar(255), logradouro varchar(255) not null, primary key (id));
create table pessoa (data_nascimento date, create_date timestamp(6), endereco_fk bigint, id bigint not null, last_modified_date timestamp(6), cpf varchar(14) unique, telefone varchar(16) unique, email varchar(100) unique, nome varchar(100) not null, create_user varchar(255), last_modified_user varchar(255), primary key (id));
...
alter table if exists eds_password_reset_token add constraint FKf7bfu7bbxn7grlsn1ehmxdh5p foreign key (user_id) references eds_user;
